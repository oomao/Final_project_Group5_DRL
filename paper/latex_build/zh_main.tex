...
\begin{table*}[t]
\centering
\caption{跨環境總表 (env\_native\_mean,n=5;\textbf{粗體} 代表相對 B0 之顯著勝出;\emph{斜體} 代表相對 B3-no-memory 之顯著敗退)}
\small
\begin{tabular}{@{}lllll@{}}
\toprule
...
\begin{table*}[t]
\centering
\caption{B3-hermes-full vs B0-env-native (主要假設驗證)}
\small
...
\begin{table*}[t]
\centering
\caption{B3-hermes-full vs B3-no-memory (記憶效應)}
\small
\begin{tabular}{@{}llllll@{}}
\toprule
...
\PaperFig{figures/fig7_part2_hermes_vs_b0.png}{Part 2 主結果:三種 DQN 變體、四個環境下之 B3-hermes-full 對 B0-env-native。MountainCar 在三種變體下皆呈現 Hermes 勝出;LunarLander 在任一變體下皆未勝出。}

\begin{table*}[t]
\centering
\caption{Part 2------B3-hermes-full vs B0-env-native (每格 n=5)}
\small
...
\PaperFig{figures/fig8_part2_hermes_robustness.png}{各環境下 B3-hermes-full 於三種 DQN 變體之平均;環境內之差異未達統計顯著。}

\begin{table*}[t]
\centering
\caption{B3-hermes-full 各變體平均 + 兩兩 Mann-Whitney U p 值}
\small
\begin{tabular}{@{}lllllll@{}}
\toprule
...
本研究之所有實驗產出皆公開於 \url{https://github.com/oomao/Final_project_Group5_DRL}:
...
\item
  原始碼:\url{hermes_dqn/}
\item
  編排器:\url{scripts/run_full_experiment.py}
\item
  每次訓練之設定與獎勵原始碼:\url{runs/final*/}
\item
  各環境比對報告:\url{reports/final*/comparison_report.md}
\item
  跨環境整合分析:\url{reports/integration/4env_integration.md}
\end{itemize}
